...
def _extract_hint(field):
    ...
    if '//' in line:
        ...
        # Parse key="value" pairs, allowing multiple values for the same key
...
def _is_enum_type(type_obj) -> bool:
    ...
    if type_obj.kind == getattr(TypeKind, 'ENUM'):
        return True
    # Handle typedef'd enums
    if type_obj.kind == getattr(TypeKind, 'TYPEDEF'):
        underlying = type_obj.get_canonical()
        return underlying.kind == getattr(TypeKind, 'ENUM')
    return False
...
def _get_enum_underlying_type(type_obj) -> Optional[str]:
    """Get the underlying type of an enum"""
    def get_type_from_enum_decl(decl):
        # First check for explicit underlying type in the enum declaration
        tokens = [t.spelling for t in decl.get_tokens()]
        try:
            colon_idx = tokens.index(':')
            if colon_idx < len(tokens) - 1:
                underlying = tokens[colon_idx + 1]
                return underlying
        except ValueError:
            pass

        # Fallback to the enum_type property
        if hasattr(decl, 'enum_type'):
            enum_type = decl.enum_type.spelling
            return enum_type
        return None

    # Direct enum type
    if type_obj.kind == getattr(TypeKind, 'ENUM'):
        return get_type_from_enum_decl(type_obj.get_declaration())

    # Typedef pointing to an enum
    if type_obj.kind == getattr(TypeKind, 'TYPEDEF'):
        underlying = type_obj.get_canonical()
        if underlying.kind == getattr(TypeKind, 'ENUM'):
            return get_type_from_enum_decl(underlying.get_declaration())

    # Elaborated type (e.g. enum class)
    if type_obj.kind == getattr(TypeKind, 'ELABORATED'):
        underlying = type_obj.get_canonical()
        if underlying.kind == getattr(TypeKind, 'ENUM'):
            return get_type_from_enum_decl(underlying.get_declaration())
...
class StructField:
    def __init__(self, field) -> None:
        ...
        self.is_enum = False
        self.enum_type = None
        if field.type.kind == getattr(TypeKind, 'ENUM') or (
            field.type.kind == getattr(TypeKind, 'TYPEDEF') and
            field.type.get_canonical().kind == getattr(TypeKind, 'ENUM')
        ) or (
            field.type.kind == getattr(TypeKind, 'ELABORATED') and
            field.type.get_canonical().kind == getattr(TypeKind, 'ENUM')
        ):
            self.is_enum = True
            self.enum_type = _get_enum_underlying_type(field.type)
